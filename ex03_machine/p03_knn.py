...
'''
train_input   : 훈련에 사용할 입력 데이터
test_input    : 테스트에 사용할 입력 데이터
train_target  : 훈련에 사용할 정답 데이터
test_target   : 테스트에 사용할 정답 데이터
'''
printt("농어의 길이(입력:input)를 통해서 몸무게(결과:target)를 예측해 보자")
train_input, test_input, train_target, test_target \
  = train_test_split(perch_length, perch_weight, random_state=42)
printt(str(type(train_target)) + str(len(train_target)))
# print(train_target)
